...
#ifndef _MSC_VER
   #if __GNUC__
/* modified to resolve the __forceinline conflict with GNUC's internal defined __forceinline*/
/* by Edward Lei 26th April, 2014 */
...
/* commented by Edward Lei 26th April, 2014
...
static __forceinline__ uint32 crc32_update(uint32 crc, uint8 byte)
{
...
static __forceinline__ void prep_huffman(vorb *f)
{
   if (f->valid_bits <= 24) {
      if (f->valid_bits == 0) f->acc = 0;
      do {
...
static __forceinline__ void draw_line(float *output, int x0, int y0, int x1, int y1, int n)
{
   int dy = y1 - y0;
   int adx = x1 - x0;
   int ady = abs(dy);
...
static __forceinline__ void iter_54(float *z)
{
...
static int start_decoder(vorb *f)
{
   ...
   for (i=0; i < f->floor_count; ++i) {
      ...
      if (f->floor_types[i] == 0) {
         ...
         for (j=2; j < g->values; ++j) {
            /* modified to initialize low,hi to 0 by Edward Lei 26th April, 2014 */
            int low = 0,hi = 0;
...
static int vorbis_analyze_page(stb_vorbis *f, ProbedPage *z)
{
   ...
   /* commented by Edward Lei on 26th April, 2014 
   int previous =0; */
   int i,len;
   uint32 samples;
   ...
   for (i=0; i < header[26]; ++i) {
      if (packet_start) {
         ...
         if (n >= f->mode_count) goto bail;
				 /* ('previous' not really be used) commented by Edward Lei on 26th April, 2014 
...
#ifndef STB_VORBIS_NO_STDIO
int stb_vorbis_decode_filename(char *filename, int *channels, int *frequency, short **output)
{
   ...
   *frequency = v->sample_rate;   /* added by Edward Lei on 24th April, 2014 */
   *channels = v->channels;
   offset = data_len = 0;
   total = limit;
...
int stb_vorbis_decode_memory(uint8 *mem, int len, int *channels, int *frequency, short **output)
{
   int data_len, offset, total, limit, error;
   short *data;
